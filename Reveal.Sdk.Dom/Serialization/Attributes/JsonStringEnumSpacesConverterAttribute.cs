...
namespace Reveal.Sdk.Dom.Serialization.Attributes
{
    /// <summary>
    /// Converts enums that contain spaces. Each word must start with an uppercase letter.
    /// For example: Rocky Mountain
    /// </summary>
    [AttributeUsage(AttributeTargets.Enum | AttributeTargets.Property, AllowMultiple = false)]
    public class JsonStringEnumSpacesConverterAttribute : JsonConverterAttribute
    {
        public override JsonConverter CreateConverter(Type typeToConvert)
        {
            return new CustomStringEnumConverter(new EnumSpacifyNamingPolicy());
        }
    }

    internal class EnumSpacifyNamingPolicy : JsonNamingPolicy
    {
    ...
    public sealed class CustomStringEnumConverter : JsonConverterFactory
    {
        JsonNamingPolicy _namingPolicy = JsonNamingPolicy.CamelCase;

        public CustomStringEnumConverter(JsonNamingPolicy namingPolicy)
        {
        ...
        public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            return (JsonConverter)Activator.CreateInstance(
                typeof(CustomStringEnumConverter<>).MakeGenericType(typeToConvert),
    ...
    public sealed class CustomStringEnumConverter<T> : JsonConverter<T> where T : Enum
    {
        private readonly JsonNamingPolicy _namingPolicy;

        private readonly Dictionary<string, T> _readCache = new Dictionary<string, T>();
        private readonly Dictionary<T, JsonEncodedText> _writeCache = new Dictionary<T, JsonEncodedText>();
        ...
        public CustomStringEnumConverter(JsonNamingPolicy namingPolicy, JsonSerializerOptions options, object[]? knownValues)
        {
            _namingPolicy = namingPolicy;
